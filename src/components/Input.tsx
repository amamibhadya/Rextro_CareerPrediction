...
const ModernSimpleInput = forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, ...props }, ref) => {
    return (
      <input
        className={cn(
          "flex h-20 w-full rounded-xl border px-3 py-2 text-sm ring-0 focus:ring-0 transition focus-visible:outline-hidden disabled:cursor-not-allowed disabled:opacity-50",
          // Colors
          ...
          // File
          "file:border-0 file:bg-transparent file:font-medium font-fredoka file:text-foreground file:text-sm",
          className,
        )}
        ref={ref}
        type={type}
